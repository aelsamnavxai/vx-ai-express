...
export async function getKnowledgeBaseData(businessId: string): Promise<KnowledgeBaseData> {
    ...
    const documents = docSnapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
        uploadDate: (doc.data().uploadDate as Timestamp).toDate().toISOString(),
    })) as Document[];
    ...
    const metadata = metadataSnapshot.exists
        ? {
            ...
            lastUpdated: (metadataSnapshot?.data()?.lastUpdated as Timestamp).toDate().toISOString(),
        }
...
export async function handleEmailSent(body: any) {
