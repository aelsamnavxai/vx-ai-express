...
const formatForSms= (html: string) => {
    return html
...
const getFollowUpStatus = (followUpSent: string) => {
    ...
    return (
        statusMap[followUpNumber as keyof typeof statusMap] ||
...
export const processDailyFollowUps = async () => {
    ...
    for (const businessDoc of businesses.docs) {
        ...
        try {
            ...
            const format_availableDates: string[] = availableDates.map(
                (date: any): string => format(new Date(date.date), "do MMMM yyyy")
            );
