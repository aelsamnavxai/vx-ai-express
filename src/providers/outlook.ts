...
export const refreshOutlookAccessToken = async (refreshToken: string) => {
    try {
...
export const getOutlookThreadMessageId = async (accessToken: string, senderEmail: string) => {
    try {
        ...
        const sorted = response.value.sort((a: any, b: any) =>
...
export async function sendOutlookEmail({
    ...
    try {
        ...
        if (mode === 'reply' && messageId) {
            ...
            const encodeSubject = (text: string) => Buffer.from(text, 'utf8').toString();
...
async function applyCategoryToEmail(accessToken: string, messageId: string, category: string) {
    try {
        const client = Client.init({
            authProvider: (done) => done(null, accessToken),
        });
...
async function getLastSentEmailId(accessToken: string) {
    try {
        const client = Client.init({
            authProvider: (done) => done(null, accessToken),
        });
...
async function isCategoryAvailable(accessToken: string, categoryName: string) {
    const client = Client.init({
        authProvider: (done) => done(null, accessToken),
    });
    ...
    try {
        ...
        const categoryExists = categories.value.some((cat: { displayName: string }) => cat.displayName === categoryName);
