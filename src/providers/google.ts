...
export const getThreadMessageId = async (oauth2Client: any, email: string) => {
    try {
...
export async function sendGoogleEmail({
    ...
    try {
        ...
        if (mode === 'forward' && threadId) {
            ...
            const threadAttachments: Array<{ filename: string; content: string; contentType: string }> = [];
...
async function applyLabelToMessage(oauth2Client: any, messageId: string, labelName: string) {
    const gmail = google.gmail({ version: 'v1', auth: oauth2Client });
