...
import { refreshOutlookAccessToken, revokeOutlookToken } from "../providers/outlook";
import { revokeGoogleToken } from "../providers/google";
...
export const getAuthenticatedOAuth2Client = async (businessId: string) => {
    let docData = null;
    let isUser = false;
    let docRef = null;
    ...
    try {
        ...
        const authToken = docData.auth as AuthToken;
...
const acquireTokenRefreshLock = async (businessId: string) => {
...
const releaseTokenRefreshLock = async (businessId: string) => {
    const LOCK_DOC_PATH = `locks/token-refresh-${businessId}`;
    const lockRef = db.doc(LOCK_DOC_PATH);
